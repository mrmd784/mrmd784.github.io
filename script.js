async function showPlates(brand) {
    ...
    // If the brand data is an array (Toyota, BMW, etc.) - THIS PART IS NOT USED WITH YOUR CURRENT data.json
    if (Array.isArray(brandData)) {
        plateList.innerHTML = brandData.map(p => `<li>${p}</li>`).join('');
    } else {
        // Handle models as objects (Honda, Toyota, BMW, etc. with your current data.json)
